...
class BrowserService {
  ...
  async fetch(
    ...
    try {
      ...
      try {
        const retrySession = await this.createSession(sessionToken, region);
        const retryFetchUrl = region === "international" ? this.rewriteInternationalUrl(url) : url;
